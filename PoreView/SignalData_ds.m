classdef SignalData_ds < handle
    
    % SJF 1/23/17
    % running SignalData_ds(filename) will create a mean downsampled
    % version of the data, aiming for 2^22 points.  before downsampling, it
    % will butterworth 4-pole filter the data at 1/2 the sampling
    % frequency.  SignalData_ds must be called manually, and is not called
    % by PoreView
    ...
    methods
        function obj = SignalData_ds(fname, varargin)
        ...
        function buildReduced(obj, freq)
            ...
            if nargin < 2
                %freq = 10000;
                totaltime = (obj.tend-obj.tstart); % sjf
                freq = obj.nred/totaltime/2; % sjf
                display(['Filtering at ' num2str(freq/1000) 'kHz and sampling at approximately ' num2str(obj.nred/totaltime/1000) 'kHz'])
            end
            % make filters
            ...
            while curind < obj.ndata
                ...
                if nhalve > 0
                    ...
                    dr = reshape(d,[nd/2 2*numel(d)/nd]); %sjf
                    ...
                    d = reshape([mean(dr)],[2*numel(d)/(nd*size(d,2)), size(d,2)]); % sjf
