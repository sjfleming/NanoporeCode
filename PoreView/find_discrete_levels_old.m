...
%         [~,p,~] = kstest2(current(ts_ind1:deriv_peak_locs(sort_ind(num))),current(deriv_peak_locs(sort_ind(num)):tf_ind1));
    ...
    %% go through each level Laszlo-style and see if there is a missed transition that lacks a sharp derivative
    % ("decoding long nanopore sequencing reads of ..." Nature Biotechnology, Supplement page 6
    
    log_prob = @(x,ind) (ind-1)*log(std(x(1:ind))) + (numel(x)-ind)*log(std(x(ind:end))) ...
        - (numel(x)-1)*log(std(x(1:end)));
    
    % loop through each level and look for missed transitions
    for i = 1:numel(level_currents)
        
        [~,level_inds(1)] = find(time>=level_timing(i,1),1,'first');
        [~,level_inds(2)] = find(time>=level_timing(i,2),1,'first');
        trans_ind = fminbnd(@(t) log_prob(current(level_inds(1):level_inds(2)),t), level_inds(1), level_inds(2));
        laszlo_value = log_prob(current(level_inds(1):level_inds(2)),trans_ind);
        % if the laszlo prob value is less than -50, designate a new level
        if laszlo_value <= -50
