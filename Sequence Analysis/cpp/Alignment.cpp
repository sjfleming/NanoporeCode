 ...
 * a bit on the technical side, I reckon.
 *
 *========================================================*/
...
Alignment::Alignment(EventData& ed, Sequence& seq, AlignParams& par) : event(&ed),
            sequence(&seq), params(par)
{
    ...
    // if there is no existing reference alignment
    if (event->ref_index.size() == 0 || params.stripe_width == 0)
    {
        // override
        cout << "FA!";
        cout.flush();
        params.stripe_width = 0;
    }

    // full-frame, if not specified
    if (params.stripe_width == 0)
        params.stripe_width = event->length+1;
    
    params.lik_skip = log(event->model.skip_prob);
    params.lik_stay = log(event->model.stay_prob);
    params.lik_extend = log(event->model.extend_prob);
}
...
{
    // now populate the thing when constructed
    while (scores.back()->col < sequence->length)
        fillColumn();
}

void Alignment::fillColumnsBack()
{
    while (sequence->length+scores_back.back()->col > 0)
        fillColumnBack();
    
    if (scores_back.back()->maxScore.score > scores.back()->maxScore.score + 0.1)
        cerr << "Back score exceeds forward score by " <<
                scores_back.back()->maxScore.score - scores.back()->maxScore.score << endl;
}
...
double Alignment::fillColumn()
{
    ...
    int curwid = params.stripe_width;
    ...
    for (int i=i0; i<=i1; i++)
    {
        ...
        if (i >= p0 && i <= p1)
        {
            liks[L_SKIP] = prevlik[i] + params.lik_skip;
        }
        else
        {
            liks[L_SKIP] = params.lik_skip;
        ...
        {
            // start a new stay
            liks[U_STAY] = curlik[i-1] + lobs + params.lik_stay;
            // or a new stay via insertion
            liks[U_INSERT] = curlik[i-1] + params.lik_insert;
            // or extend a stay, from stay matrix to stay matrix
            liks[U_EXTEND] = curstay[i-1] + lobs + params.lik_extend;
            // or extend a stay without counting observation prob.
            liks[U_BADEXTEND] = curstay[i-1] + lobs + params.lik_extend;
        }
...
double Alignment::fillColumnBack()
{
    ...
    int refind = sequence->length+colind+1;
    if (refind <= 0)
        return 0;
    ...
    for (int i=i0; i<=i1; i++)
    {
        ...
        {
            // start a new stay
            liks[U_STAY] = curlik[i-1] + curobs[i-1] + params.lik_stay;
            // or a new stay via insertion
            liks[U_INSERT] = curlik[i-1] + params.lik_insert;
            // or extend a stay, from stay matrix to stay matrix
            liks[U_EXTEND] = curstay[i-1] + curobs[i-1] + params.lik_extend;
            // or extend a stay without counting observation prob.
            liks[U_BADEXTEND] = curstay[i-1] + curobs[i-1] + params.lik_extend;
        }
...
{
    // uuhhhh this code is pretty gosh-darn hacky and should not see the
    ...
    //cout << "Mut start,count: " << mut.start << "," << mut.nmut << endl;
    //for (int i=0; i<mutseq.length; i++)
    //    if (origseq->states[i] != mutseq.states[i])
    //        mexPrintf("Difference in col %d from %d to %d\n",i+1,origseq->states[i],mutseq.states[i]);
    ...
    fillColumns(mut.nmut + 6);
    
    //for (int i=0; i<5; i++)
    //    cout << "last col - " << i << ": " << scores[scores.size()-1-i]->col << endl;
    // and calculate the difference in score. oh god this hurts
    int refind = mut.start+mut.nmut+1; // this is the fake forward ind that we need to find
    //cout << "refind: " << refind << endl;
    int fwdind = scores.size()-1;
    //cout << "fwdind start: " << fwdind << endl;
    while (scores[fwdind]->col > refind && fwdind >= 0)
        fwdind--;
    //cout << "fwdind: " << fwdind << endl;
    
    //cout << "fwd col: " << scores[fwdind]->col << " & back col: " << scores_back[backind]->col << endl;
    ...
    int backind = sequence->length-refind+1;
    ...
    fillColumns();
    cout << "fwdscore: " << scores.back()->maxScore.score << endl;
    while (scores_back.size() > 1)
        scores_back.pop_back();
    fillColumnsBack();
    cout << "backscore: " << scores_back.back()->maxScore.score << endl;
    */
