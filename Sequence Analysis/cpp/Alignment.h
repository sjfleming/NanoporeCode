...
struct AlignColumn
{
    ...
    // the actual data, allocated as one chunk
    double* _data;
    
    // create an empty one
    AlignColumn(int num, int ind0, int ind1) : length(num), i0(ind0), col(ind1)
    {
...
public:
    ...
    Alignment(EventData& ed, Sequence& seq, AlignParams& par);
    ...
    // change stripe width
    void setStripeWidth(int sw)
    {
        this->params.stripe_width = sw;
    }
    ...
    double columnMax(int refind)
    {
        ...
        int raf = refind;
        int rab = sequence->length-refind+1;
